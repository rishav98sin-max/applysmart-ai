...
def generate_cover_letter(
    ...
    from agents.llm_client    import chat_deepseek, chat_quality, last_llm_source
    ...
    for attempt in range(retries):
        try:
            ...
            # On attempt 0 we try DeepSeek (better instruction-following,
            # less boilerplate); on empty/truncated response we drop straight
            # to Groq. Subsequent retries skip DeepSeek entirely — we already
            # know it's not producing valid output for this artifact this run,
            # and retries benefit most from Groq's larger token budget.
            if attempt == 0:
                raw = chat_deepseek(
